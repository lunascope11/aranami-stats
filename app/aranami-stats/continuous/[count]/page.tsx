...
export default async function ContinuousCountDetailPage({
  params,
}: {
  params: Promise<{
    count: string;
  ...
  const segments =
    stats.concurrentContinuousSegments.filter(
      (segment) =>
        segment.participantCount === participantCount,
    );
      ...
      <div className="mx-auto max-w-6xl">
        <header className="mb-10">
          ...
          <h1 className="mt-2 text-3xl font-bold">
            {participantCount}명 동시 방송 연속 기록
          </h1>

          <p className="mt-3 text-sm leading-6 text-zinc-400">
            정확히 {participantCount}명이 같은 조합으로
            끊기지 않고 방송한 연속시간 전체 순위입니다.
          </p>
        </header>
        ...
        <div className="space-y-4">
          {segments.map((segment, index) => {
            const isNotable =
              participantCount >= 7 &&
              segment.durationMs >= LONG_SEGMENT_MS
